function r = ADMM3D(kspData, iO, maxIter, mu, lambda, wPhi, wTV)
...
r = bifftn(kspData);
d = dTrafoIso3D(r, wPhi, wTV);
k = zeros(size(d));
a = 1;

rhs = @(d,k) 2*bifftn(kspData) + adTrafoIso3D(mu*d + k, wPhi, wTV);

for jj = 1:maxIter

    d = dUpdateIso3D_w(r, k, lambda, wPhi, wTV, mu);
    
    rNew = bifftn(iO.*bfftn(rhs(d,k)));
    
    kNew = k + mu*(d - dTrafoIso3D(rNew, wPhi, wTV));
